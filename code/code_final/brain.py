...
# --- 1. DEFINITION DU MODELE (DOIT ETRE IDENTIQUE A TON TRAIN) ---
...
try:
    # On charge le fichier généré par ton script d'entraînement
    model.load_state_dict(torch.load("smart_dqn.pth", map_location=torch.device('cpu')))
    model.eval() # Mode évaluation
    print("✅ Cerveau Dueling DQN chargé avec succès !")
except Exception as e:
    print(f"❌ ERREUR : Impossible de charger 'smart_dqn.pth'.\nAs-tu bien lancé le script d'entraînement avant ?\nErreur: {e}")

# --- 3. FONCTIONS REFLEXES (POUR NE PAS ETRE NUL) ---
...
def predict():
    try:
        ...
        if block_move is not None:
            print(f"🛡️ BLOCAGE D'URGENCE : Colonne {block_move}")
